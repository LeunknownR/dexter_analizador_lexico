...
import { STATES } from "core/lexic/constants/states";

const CAPTIONS = {
	[STATES.RESERVED_WORD]: (
		<ReservedWordIdentifier caption="IDENTIFICADOR DENTRO DE PALABRAS RESERVADAS" />
	),
	[STATES.BOOLEAN]: (
		<ReservedWordIdentifier caption="IDENTIFICADOR DENTRO DE BOOLEANOS" />
	),
};
...
const OperationComponent = ({
	...
	return (
		<OperationContainer>
			...
			/>
			{operations.map(
				({ state, newState, char: { group, value } }, idx) => (
					<OperationRow
						state={state}
						newState={newState}
						group={group}
			...
			{reservedWordType && CAPTIONS[reservedWordType]}
		</OperationContainer>
	);
};
