...
const test = () => {
	try {
		const [_, debugLog] = lexicAnalyzer(`ssd if+asdsa ofascxc( 23   `);
...
const test3 = () => {
	try {
		...
		const [componentList, debugLog] = lexicAnalyzer(
			manuelExpressions[expressionIdx]
		);
...
const showConsoleDebugLog = debugLog => {
	console.table(
		debugLog.reduce((simplifiedLog, { operations, title }) => {
			let newSimplifiedLog = [...simplifiedLog, " "];
			newSimplifiedLog = [
				...newSimplifiedLog,
				`** ${title.lexeme} - ${title.token} **`,
			];
			newSimplifiedLog = [
				...newSimplifiedLog,
				...operations.map(({ state, char, newState }) => {
